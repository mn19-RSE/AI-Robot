...
void setup() {
  ...
  motor2.velocity_limit = motor2_vel_limit;  // angle loop velocity limit
  motor2.voltage_limit = motor2_V_limit;     // default voltage_power_supply
...
float pos1 = 10;  // previously: 6.28, 5, 2, -2, 5, 12, -3, -8, -5, 1, 10
float pos2 = -10;  // previously 3.14, 4, 1, -1, 4, 10, -2, -10, -4, 0, -10
int loopTime = 5000;
...
void loop() {
  ...
  while (millis() < ((1 - (motor_slow_speed / motor_quick_speed)) * loopTime)) {
    ...
    motor2.velocity_limit = motor_slow_speed;
    motor1.loopFOC();
    motor2.loopFOC();
    motor1.move(pos1);
    motor2.move(pos1);
